...
public class Teleporting : NetworkBehaviour
{
    ...
    public void teleport_to_billiaer()
    {
        RpcTeleport(billiard_teleport.transform.position);
    }

    public void teleport_to_beerpong()
    {
        RpcTeleport(beerpong_teleport.transform.position);
    }

    public void teleport_to_karaoke()
    {
        RpcTeleport(karaoke_teleport.transform.position);
    }

    public void teleport_to_uno()
    {
        RpcTeleport(uno_teleport.transform.position);
    }

    [Rpc(RpcSources.All, RpcTargets.InputAuthority)]
    void RpcTeleport(Vector3 targetPosition)
    {
        Debug.Log($"Teleporting to {targetPosition}");

        // Keep camera height same
        targetPosition.y = camera_gameobject.transform.position.y;
        camera_gameobject.transform.position = targetPosition;
    }
}
